...
public abstract partial class SharedTetherGunSystem
{
    private void InitializeForce()
    {
        ...
        SubscribeLocalEvent<TetherForseCombineComponent, AfterInteractEvent>(OnForceRanged);
        SubscribeLocalEvent<TetherForseCombineComponent, ActivateInWorldEvent>(OnForceActivate);
        //SubscribeLocalEvent<TetherForseCombineComponent, ActivateInWorldEvent>(OnForceActivate);
    }
    ...
    /// SHIT COPY CODE
    ///
    private void OnForceActivate(EntityUid uid, TetherForseCombineComponent component, ActivateInWorldEvent args)
    {
        if (!args.Complex)
            return;
    ...
    private void OnForceRanged(EntityUid uid, TetherForseCombineComponent component, AfterInteractEvent args)
    {
        if (IsTethered(component))
        {
            if (!args.ClickLocation.TryDistance(EntityManager, TransformSystem, Transform(uid).Coordinates,
    ...
    private bool IsTethered(TetherForseCombineComponent component)
    {
        return component.Tethered != null;
    }
}
